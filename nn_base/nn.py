...
class NN(nn.Module):
    ...
    def save_to_disk(self, filepath=config.NN_SAVE_PATH):
...
def apply_nn(data, label, filepath=config.NN_SAVE_PATH):
...
def test_nn(data, label, filepath=config.NN_SAVE_PATH):
    ...
    net = NN(16).load_from_disk(filepath)
...
def get_prediction(instance):
    filepath = os.path.join(config.PROJECT_ROOT, 'nn_base', 'net_v1.model')
    net = NN(16).load_from_disk(filepath)

    instance = torch.tensor(np.array([instance]), dtype=torch.float32)

    output_test = net(instance).detach().apply_(lambda x: 0.5 * round(x / 0.5))
    output_test = torch.argmax(output_test, dim=1)[0]

    return output_test.item()
...
def main():
    ...
    # apply_nn(data_frame, label[config.TARGET_COLUMN_NAME], filepath=config.NN_SAVE_PATH_V1)
    test_nn(data_frame, label[config.TARGET_COLUMN_NAME], filepath=config.NN_SAVE_PATH_V1)
